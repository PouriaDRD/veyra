...
class Search:
    ...
    def __post_init__(self) -> None:
        """Validate and normalize search state."""
        ...
        if self.started_at is not None:
            self.started_at = ensure_utc_datetime(
                self.started_at,
        ...
        if self.completed_at is not None:
            self.completed_at = ensure_utc_datetime(
                self.completed_at,
    ...
    def transition_to(
        ...
        *,
        ...
        """
        Move the search to a valid non-failure lifecycle state.

        Failure transitions must use `fail()` so a diagnostic reason
        is always preserved.
        """

        if status is SearchStatus.FAILED:
            raise ValueError(
                "Use fail() to transition a search to failed.",
            )

        if status is self.status:
            return

        timestamp = self._prepare_transition(
            status,
    ...
    def fail(
        ...
        *,
        ...
        """Mark the search as failed with a diagnostic reason."""
    ...
    def _prepare_transition(
        ...
        *,
        ...
        """Validate a transition and return its normalized timestamp."""
    ...
    def _validate_state(self) -> None:
        """Validate invariants associated with the current search status."""

        if self.status is SearchStatus.CREATED:
            if self.started_at is not None:
                raise ValueError(
                    "created search must not have started_at.",
                )

            if self.completed_at is not None:
                raise ValueError(
                    "created search must not have completed_at.",
                )

            if self.failure_reason is not None:
                raise ValueError(
                    "created search must not have failure_reason.",
                )
        ...
        if self.status in {
            ...
            if self.started_at is None:
                raise ValueError(
                    "active search must have started_at.",
                )

            if self.completed_at is not None:
                raise ValueError(
                    "active search must not have completed_at.",
                )

            if self.failure_reason is not None:
                raise ValueError(
                    "active search must not have failure_reason.",
                )
        ...
        if self.status is SearchStatus.COMPLETED:
            if self.started_at is None:
                raise ValueError(
                    "completed search must have started_at.",
                )

            if self.completed_at is None:
                raise ValueError(
                    "completed search must have completed_at.",
                )

            if self.failure_reason is not None:
                raise ValueError(
                    "completed search must not have failure_reason.",
                )
        ...
        if self.status is SearchStatus.FAILED:
            if self.completed_at is None:
                raise ValueError(
                    "failed search must have completed_at.",
                )

            if self.failure_reason is None:
                raise ValueError(
                    "failed search must have failure_reason.",
                )
...
class SearchCandidate:
    ...
    Candidate processing state is search-specific and therefore does not
    belong to the global Profile entity.
    """
    ...
    def attach_snapshot(
        ...
        """Attach the captured snapshot to this candidate."""
    ...
    def filter_out(
        ...
        """Exclude a candidate from scoring."""

        if self.status is not CandidateStatus.ANALYZED:
            raise ValueError(
                "Only analyzed candidates can be filtered out.",
            )
    ...
    def set_score(
        ...
        """Assign the candidate's normalized 0-10 score."""
    ...
    def _validate_state(self) -> None:
        """Validate invariants associated with candidate processing state."""

        if self.status is CandidateStatus.DISCOVERED:
            if self.snapshot_id is not None:
                raise ValueError(
                    "discovered candidate must not have snapshot_id.",
                )

            if self.score is not None:
                raise ValueError(
                    "discovered candidate must not have score.",
                )

            if self.exclusion_reason is not None:
                raise ValueError(
                    "discovered candidate must not have exclusion_reason.",
                )
        ...
        if self.snapshot_id is None:
            raise ValueError(
                "processed candidate must have snapshot_id.",
            )
        ...
        if self.status in {
            ...
            if self.score is not None:
                raise ValueError(
                    "unscored candidate must not have score.",
                )

            if self.exclusion_reason is not None:
                raise ValueError(
                    "unfiltered candidate must not have exclusion_reason.",
                )
        ...
        if self.status is CandidateStatus.FILTERED_OUT:
            if self.score is not None:
                raise ValueError(
                    "filtered candidate must not have score.",
                )

            if self.exclusion_reason is None:
                raise ValueError(
                    "filtered candidate must have exclusion_reason.",
                )
        ...
        if self.status is CandidateStatus.SCORED:
            if self.score is None:
                raise ValueError(
                    "scored candidate must have score.",
                )

            if self.exclusion_reason is not None:
                raise ValueError(
                    "scored candidate must not have exclusion_reason.",
                )
    ...
    def _validate_score(
        ...
        """Validate normalized candidate score."""
