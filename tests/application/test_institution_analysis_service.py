...
def test_english_student_institution_is_supported() -> None:
    ...
    assert fact.values == ("tehran university",)
...
def test_persian_student_institution_is_supported() -> None:
    ...
    assert fact.status is FactStatus.SUPPORTED
    assert fact.values == ("دانشگاه تهران",)


def test_historical_graduate_institution_is_supported() -> None:
    fact = institution_fact_for(
        bio="Graduate of Sharif University",
    )

    assert fact.status is FactStatus.SUPPORTED
    assert fact.values == ("sharif university",)
...
def test_two_different_explicit_institutions_are_supported_together() -> None:
    fact = institution_fact_for(
...
def test_duplicate_same_institution_is_deduplicated() -> None:
    fact = institution_fact_for(
        bio="Graduate of MIT | Student at MIT",
    )
...
def test_institution_and_occupation_are_separate_facts() -> None:
    result = ProfileAnalysisService().analyze(
        build_snapshot(
            bio="Software Engineer | Student at Tehran University",
        ),
        reference_date=REFERENCE_DATE,
    )
    ...
    assert institution.values == ("tehran university",)


def test_institution_and_employer_are_separate_facts() -> None:
    result = ProfileAnalysisService().analyze(
        build_snapshot(
            bio="Software Engineer at Acme | Student at MIT",
        ),
        reference_date=REFERENCE_DATE,
    )
